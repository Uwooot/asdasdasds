...
void CEsp::DrawPlayer(IClientEntity* pEntity, player_info_t pinfo)
{
	...
	if (GetBox(pEntity, Box))
	{
		...
		if (Menu::Window.VisualsTab.OptionsInfo.GetState() || Menu::Window.VisualsTab.OptionsInfo.GetState())
		...
		if (Menu::Window.VisualsTab.OptionsAimLine.GetState())
			DrawAimLine(pEntity);
...
// 2D Corner Esp box
void CEsp::DrawBoxCorner(IClientEntity* pEntity, CEsp::ESPBox size, Color color)
{
	float alpha = flPlayerAlpha[pEntity->GetIndex()];
...
void CEsp::DrawBoxFull(IClientEntity* pEntity, CEsp::ESPBox size, Color color)
{
	Render::Outline(size.x, size.y, size.w, size.h, color);
	Render::Outline(size.x - 1, size.y - 1, size.w + 2, size.h + 2, Color(10, 10, 10, 150)); //outline outer
	Render::Outline(size.x + 1, size.y + 1, size.w - 2, size.h - 2, Color(10, 10, 10, 150)); //outline inner
}

// Full 3D  Esp box


void CEsp::DrawBox3D(IClientEntity* pEntity, CEsp::ESPBox size, Color color)
{
	Render::Outline(size.x, size.y, size.w, size.h, color);
	Render::Outline(size.x - 1, size.y - 1, size.w + 2, size.h + 2, Color(10, 10, 10, 150)); //outline outer
	Render::Outline(size.x + 1, size.y + 1, size.w - 2, size.h - 2, Color(10, 10, 10, 150)); //outline inner
...
/*
void CEsp::DrawBox3D(Vector minin, Vector maxin, Vector pos, Color Col)
{
...
void CEsp::DrawAimLine(IClientEntity* pEntity)
{
	...
	{
		Render::Line(MidX, MidY, screen.x, screen.y, Color(255, 0, 0, 255));
		Render::Line(MidX + 1, MidY + 1, screen.x, screen.y, Color(255, 0, 0, 255));
		Render::Line(MidX + 1, MidY - 1, screen.x, screen.y, Color(255, 0, 0, 255));
		Render::Line(MidX - 1, MidY + 1, screen.x, screen.y, Color(255, 0, 0, 255));
		Render::Line(MidX - 1, MidY - 1, screen.x, screen.y, Color(255, 0, 0, 255));
	}
...
void CEsp::DrawInfo(IClientEntity* pEntity, CEsp::ESPBox size)
{
	...
	if (Menu::Window.VisualsTab.OptionsInfo.GetState() && pWeapon)
	{
...
void CEsp::DrawChicken(IClientEntity* pEntity, ClientClass* cClass)
{
	...
	if (GetBox(pEntity, Box))
	{
		...
		if (Menu::Window.VisualsTab.OptionsBox.GetIndex() > 1)
			DrawBoxCorner(pEntity, Box, Color(255, 255, 255, 255));

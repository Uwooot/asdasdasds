...
void CGUI::Draw()
{
	...
	if (ShouldDrawCursor)
	{
		...
		Render::PolygonOutline(3, MouseVt, Color(177, 247, 27, 230), Color(152, 0, 255, 240));
	}
}
...
{
	Render::Outline(window->m_x, window->m_y, window->m_iWidth, window->m_iHeight, Color(177, 247, 27, 80));
	//Render::GradientV(window->m_x + 2, window->m_y + 2, window->m_iWidth - 4, 26, Color(255, 40, 40, 255), Color(255, 70, 70, 255));
	Render::GradientV(window->m_x + 2, window->m_y + 2, window->m_iWidth - 4, 26, Color(177, 247, 27, 255), Color(82, 114, 12, 170));
	Render::Clear(window->m_x + 2, window->m_y + 2 + 26, window->m_iWidth - 4, window->m_iHeight - 4 - 26, Color(30, 30, 30, 240));
	//Render::Outline(window->m_x + 1, window->m_y + 1, window->m_iWidth - 2, window->m_iHeight - 2, Color(240, 240, 250, 255));
	Render::Outline(window->m_x + 1, window->m_y + 1, window->m_iWidth - 2, window->m_iHeight - 2, Color(177, 247, 27, 255));
	Render::Text(window->m_x + 8, window->m_y + 8, Color(255, 255, 255, 255), Render::Fonts::MenuBold, window->Title.c_str());
	Render::Clear(window->m_x + 8, window->m_y + 57, window->m_iWidth - 4 - 12, window->m_iHeight - 65, Color(30, 30, 30, 255));

	//Tab
	Render::GradientV(window->m_x + 8, window->m_y + 1 + 27, window->m_iWidth - 4 - 12, 29, Color(20, 20, 20, 240), Color(30, 30, 30, 245));
	int TabCount = window->Tabs.size();
	if (TabCount) // If there are some tabs
	{
		int TabSize = (window->m_iWidth - 4 - 12) / TabCount;
		for (int i = 0; i < TabCount; i++)
		{
			...
			{
				Render::GradientV(window->m_x + 8 + (i*TabSize), window->m_y + 1 + 27, TabSize, 29, Color(86, 86, 86, 255), Color(60, 60, 60, 255));
			}
